...
export default function ProfileSidebar({ isOpen, onClose, user, onLogout }: ProfileSidebarProps) {
    ...
    return (
        <div className={`fixed inset-0 z-50 transition-all duration-300 ${isOpen ? "visible" : "invisible pointer-events-none"}`}>
            ...
            {/* Sidebar Panel */}
            <div className={`absolute top-0 right-0 h-full w-full sm:max-w-sm bg-[#111111] shadow-2xl transform transition-transform duration-500 ease-[cubic-bezier(0.32,0.72,0,1)] flex flex-col ${isOpen ? "translate-x-0" : "translate-x-full"}`}>
                ...
                <div className="flex-1 overflow-y-auto">
                    {user ? (
                        <>
                            {/* User Card */}
                            <div className="p-6 text-center border-b border-white/10 bg-[#111111] relative overflow-hidden">
                                <div className="w-24 h-24 mx-auto bg-gray-800 rounded-full flex items-center justify-center text-4xl shadow-lg shadow-black/20 border-4 border-gray-700 mb-4 overflow-hidden relative z-10">
                                    {user.gender === "Female" ? "👩" : "👨"}
                                </div>
                                <h3 className="font-bold text-xl text-white relative z-10">{user.name || "User"}</h3>
                                <p className="text-gray-400 text-sm mt-1 relative z-10">{user.email || "No email linked"}</p>
                            ...
                            <div className="p-4 space-y-2">
                                ...
                                <Link href="/profile" onClick={onClose} className={`flex items-center gap-4 px-4 py-4 rounded-xl hover:bg-white/5 text-gray-300 transition-all duration-500 delay-150 group border border-transparent hover:border-white/10 ${isOpen ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}>
                                    <span className="w-10 h-10 rounded-full bg-white/5 text-gray-300 flex items-center justify-center group-hover:bg-white group-hover:text-black transition-colors shadow-sm">
                                        <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" /></svg>
                                    </span>
                                    <div className="flex-1">
                                        <span className="block font-semibold text-white group-hover:text-white transition-colors">My Profile</span>
                                        <span className="block text-xs text-gray-500 mt-0.5">Edit details & documents</span>
                                    </div>
                                    <svg className="w-5 h-5 text-gray-500 group-hover:text-white group-hover:translate-x-1 transition-all" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" /></svg>
                                </Link>

                                <Link href="/about" onClick={onClose} className={`flex items-center gap-4 px-4 py-4 rounded-xl hover:bg-white/5 text-gray-300 transition-all duration-500 delay-200 group border border-transparent hover:border-white/10 ${isOpen ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}>
                                    <span className="w-10 h-10 rounded-full bg-white/5 text-gray-300 flex items-center justify-center group-hover:bg-white group-hover:text-black transition-colors shadow-sm">
                                        <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                                    </span>
                                    <div className="flex-1">
                                        <span className="block font-semibold text-white group-hover:text-white transition-colors">About Sangam</span>
                ...
                {user && (
                    <div className="p-6 border-t border-white/10 bg-[#111111]">
                        <button
                            onClick={onLogout}
                            className="w-full flex items-center justify-center gap-2 py-3.5 rounded-xl border border-red-500/20 text-red-400 hover:bg-red-500/10 hover:border-red-500/30 transition-all font-semibold text-sm group"
                        >
